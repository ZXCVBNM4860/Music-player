...
class UserBubble(QFrame):
    ...
    def _build_ui(self, text: str):
        ...
        label.setWordWrap(True)
        label.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
        label.setStyleSheet("color: #ffffff; font-size: 10pt; line-height: 1.5;")
...
class AIBubble(QFrame):
    ...
    def _build_ui(self, text: str, reasoning: str):
        ...
        if reasoning:
            ...
            t_label.setStyleSheet("color: #cccccc; font-size: 9pt; line-height: 1.5;")
        ...
        for part in parse_markdown(text):
            if part[0] == 'text':
                txt = part[1].strip()
                if txt:
                    layout.addWidget(TextBlockWidget(txt))
            elif part[0] == 'code':
...
class DeepSeekChat(QMainWindow):
    ...
    def _add_user_message(self, text: str):
        self.chat_layout.insertWidget(self.chat_layout.count() - 1, UserBubble(text))
        QTimer.singleShot(50, self._scroll_to_bottom)

    def _add_ai_message(self, text: str, reasoning: str = ""):
        self.chat_layout.insertWidget(self.chat_layout.count() - 1, AIBubble(text, reasoning))
        QTimer.singleShot(50, self._scroll_to_bottom)
