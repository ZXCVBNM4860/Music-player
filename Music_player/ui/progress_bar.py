...
class DownloadProgressBar(QWidget):
    ...
    @staticmethod
    def _progress_stylesheet(percent: int) -> str:
        text_color = "#ffffff" if percent < 48 else "#0a0a0a"
        return (
            f"QProgressBar {{border: 1px solid #333333;border-radius: 3px;"
            f"background-color: #1a1a1a;text-align: center;color: {text_color};}}"
            f"QProgressBar::chunk {{background-color: #ffffff;border-radius: 2px;}}"
        )

    def _update_progress_ui(self, percent: int, status_text: str, detail_text: str):
        self.progress.setValue(percent)
        self.progress.setStyleSheet(self._progress_stylesheet(percent))
    ...
    def set_progress(self, current: int, total: int):
        if total > 0:
            ...
            self._update_progress_ui(percent, f"{current} / {total}", f"{percent}%")
