...
from PyQt6.QtWidgets import (
    ...
    QToolBar, QApplication, QMessageBox, QMenu, QToolButton, QLabel,
    QPushButton
)
from PyQt6.QtCore import Qt, QSettings, QTimer, QObject, pyqtSignal, QThread
    ...
    def run(self):
        try:
            ...
            if self.item_type == "mv":
                u = self.api.get_mv_download_url(self.item_id)
            else:
                u = self.api.get_download_url(self.item_id, 128000)
        except Exception:
            u = None
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        if current == "system":
            actual = detect_system_theme(QApplication.instance())
        else:
            actual = current
        try:
    ...
    def _build_menu(self):
        ...
        act_local = QAction(i18n.tr("local_player_menu"), self)
        act_local.setShortcut("Ctrl+L")
        view_menu.addSeparator()
        view_menu.addAction(act_local)
        act_local.triggered.connect(self._open_local_player)
    ...
    def _connect_signals(self):
        ...
        self.download_panel.local_player_clicked.connect(self._open_local_player)
    ...
    def _on_theme_selected(self, theme: str):
        ...
        if theme == "system":
            actual = detect_system_theme(app)
        else:
            actual = theme
    ...
    def _reload_ui(self):
        ...
        if theme == "system":
            actual_theme = detect_system_theme(QApplication.instance())
        else:
            actual_theme = theme
