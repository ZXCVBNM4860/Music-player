...
def retry_on_error(max_retries: int = Config.MAX_RETRY, delay: float = 1.0):
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            raise APIError(f"请求失败（已重试 {max_retries} 次）: {last}") from last
        return wrapper
    return decorator
...
class APIClient:
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> dict:
        ...
        if code != 200:
            ...
            if 500 <= code < 600:
                raise RetryableAPIError(f"服务端错误 {code}: {msg}")
            if code == 404:
                raise APINotFoundError(f"资源不存在 {code}: {msg}")
            raise APIError(f"API 返回错误码 {code}: {msg}")
    ...
    def download_file(self, url: str, output_path: Path,
                      ...
                      max_retries: int = Config.MAX_RETRY) -> bool:
        for attempt in range(max_retries):
            try:
