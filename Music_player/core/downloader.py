...
class DownloadTask(QThread):
    ...
    def _get_task_items(self) -> List[Dict[str, Any]]:
        if self.task_type == 1:  # 单曲
            return [{"id": self.task_id, "name": None, "artists": None, "type": "song"}]
    ...
    def _prepare_item(self, item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            if item_type == "song":
                return self._prepare_song(item_id, item_name, item.get("artists"))
            elif item_type == "mv":
    ...
    def _prepare_song(self, song_id: str, song_name: Optional[str], artists: Optional[List[str]] = None) -> Optional[Dict[str, Any]]:
        if not song_name:
            try:
        ...
        song_name = build_song_filename(song_name, artists or [], song_id)
        ext = "flac" if self.bitrate == "flac" else "mp3"
